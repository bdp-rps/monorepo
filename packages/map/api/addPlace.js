...
const placePath = 'packages/map/src/places/'
const extension = '.json'

import escapePlaceName from '../src/utils/escapePlaceName'
...
export default async function addPlace(
  ...
  const fileName = escapePlaceName(place.name)
  const submitDate = Date.now()

  const { changes, ...placeData } = place
  const branchName =
    fileName +
    '_' +
    hash(JSON.stringify(placeData)) +
    '_' +
    hash(placeData.content)
...
Eingereicht von: ${submitter} (${submitterMail})`
  ...
  const fetchSha = await fetchGetFileSha(
    auth,
    branchName + ':' + placePath + fileName + extension
  )
