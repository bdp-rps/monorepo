...
import Teilnahme from '../../templates/Teilnahme'
...
export default function Page({ defaultData, defaultGenerated }) {
  ...
  const fileName = 'Teilnahmebestätigung_' + event.value + '_' + name.value
  ...
  if (generated) {
    ...
    const body = [
      'Hey Philipp,',
      '',
      'Ich brauche eine Teilnahmebestätigung mit folgenden Daten:',
      '',
    ...
    return (
      <Template>
        <Layout paddingTop={10} paddingBottom={20} space={8} grow={1}>
          <Text variant="title">Antrag auf Teilnahmebestätigung</Text>
          <Box space={4} alignItems="flex-start">
            <Box>
              <Button
                href={`mailto:philipp@bdp-rps.de?subject=Teilnahmebestätigung ${
                  event.value
        ...
        <Layout paddingTop={10} paddingBottom={20} space={8}>
          ...
          <Box
            ...
            onReset={(e) => {
              ...
              router.push('/formulare/teilnahme')
            }}
            onSubmit={(e) => {
              e.preventDefault()
